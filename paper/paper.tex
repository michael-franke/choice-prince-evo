...
unconscious) representation of both beliefs and the quality of choice options, the question of which such
representations are ecologically valuable (i.e., lead to high fitness) is central and has been
subject of recent interest also in economics literature \citep[e.g.,][]{algweib13,DekElyYlan07,RobSam11}. Na\"ive intuition might convince us that preferences that veridically
represent the actual, true fitness measure that evolutionary selection is based upon must
surely be the best. We demonstrate that, if we take into account the different ways agents may cope with
...
environment. Concretely, we look at evolutionary ``meta-games'' whose ``acts'' are choice mechanisms and whose ``payoffs'' represent the average expected
fitness that different choice mechanisms would accrue when playing
...
Section \ref{sec:rati--subj} overviews different accounts of rationality in decision making and relates the approach we take here to the existing literature on the topic. Section \ref{sec:basic-notions} introduces the various decision- and game-theoretic notions that will be later used in the evolutionary analysis. Sections \ref{sec:model} and \ref{sec:basic-model-1} define the evolutionary framework, and Section \ref{sec:results:-basic-model} presents the main results of the paper. Section \ref{sec:extensions} extends the evolutionary analysis by introducing more choice mechanisms in the model and by investigating the evolutionary performances of different choice mechanisms when facing arbitrary decision problems. Finally, in Section \ref{sec:conclusion} we comment on the results achieved and future perspectives.
...
% selection function (maxmin expected utility) gives rise to an intersting parallel with
