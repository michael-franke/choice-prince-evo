...
\date{current version: November 2nd 2015}
...
This makes two contributions to the literature on evolution of choice mechanisms: one
...
competition of choice mechanisms in a ``meta-game'' that consists of the average expected
payoffs of choice mechanisms when playing arbitrary games (from a given class). In this sense,
a meta-game captures (the modeller's) assumptions about the relevant statistics of the
...
freedom to adjust beliefs and preferences at will. But there are many ways in which
...
maximization will survive in the long run.\todo{@Paolo: please add references}
...
\citep[e.g.][]{Savage1951:The-theory-of-s,LoomesSugden1982:Regret-Theory:-}\todo{add
  references?}\ can be adaptive and even outperform objective, veridical representations of the
...
choices of all agents that are, in some sense or other, locked-in. We will look at the former
here; the latter will be relevant only later, when turn to evolutionary competition between
choice mechanisms in Section~\ref{sec:model}.\footnote{We make a subtle distinction here
  between \emph{choice rules} and \emph{choice mechanisms}: the former refers to a function of
  mapping (subjective) preferences and uncertainty unto choices of acts; the latter is more
  encompassing and contains the choice rule, together with the particular way of forming or
  representing preferences and uncertainty.}
...
$j \neq i$ be the set of all \emph{profiles} of acts for all players except $i$. If player $i$
has a belief $\mu_i \in \Delta(A_{-i})$ about the behavior of his opponents, he chooses
rationally only if his choice $a^*_i$ maximizes expected utility
...
asking: what is the worst possible out come for each row act? It is $0$ in either
...
\begin{example}[Coordination Game]
  ...
  shows the pairwise regrets and the regrets of each row-player action. We have
  $\text{reg}(I)=2$, because when the other player chooses action $I$ the pairwise regret
  $reg(I,I)$ is $0$ since $I$ is a best reply to $I$, and when the other player chooses $II$
  the pairwise regret $reg(I, II)$ is $2$ since $\pi_i(II,II)-\pi_i(I,II)=2$. With the same
  reasoning, $\text{reg}(II)=1$. A regret minimizing player would therefore choose act
  $II$. $ \medsquare $
...
Regret minimization is intimately related to the maximin rule. Let the \emph{negative regret
  transform} of a strategic game $G$ be the game $G'$ derived from $G$ by replacing the payoff
...
\todo[inline]{Minor edits up to here}
...
Before we turn to evolutionary competition between choice mechanisms, we should justify our
focus on the two choice rules. In the following we argue that both the minimax rule, as well as
the Laplace rule can be viewed as the same choice rule under different representations of
uncertainty. 
...
In line with traditional evolutionary game theory, we model a population of rather unsophisticated and myopic agents. %that possibly differ from each other with respect to two components: the player type and the epistemic type. The players 
...
In line with traditional evolutionary game theory, the idea of this work is to model a population of rather unsophisticated and myopic agents. %that possibly differ from each other with respect to two components: the player type and the epistemic type. The players 
The players encode both a psychological subjective representation of the interaction (player type) and a conceptualization of uncertainty (epistemic type), but they are too simple to have any learning or belief updating procedure. From this point of view, we can take the learning process away from natural selection and focus on player types and epistemic types as the target of evolutionary pressure.
