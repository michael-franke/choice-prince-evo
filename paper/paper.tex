...
Rabin, etc.).\todo{@Paolo: please include proper references}
...
We focus here on symmetric games with two players. A two-player game is symmetric if players
have the same action set, $A_1 = A_2$, and payoffs are symmetric:
...
for player choosing the row against an opponent who chooses the column acts. Since the game is
symmetric, it suffices to give just these numbers. A rational player would choose $I$ exactly
when he beliefs that his opponent will choose $I$ with a probability at least
$\nicefrac{2}{3}$.

If an agent does not have a concrete probabilistic belief, the \emph{minimax rule} would tell
him to choose act $\act^*$ only if it maximizes payoffs for all worst case scenarios, i.e.,
$\act^* \in \argmax_{\myact{i} \in \Acts_i} \min_{\myact{j} \in \Acts_j} \pi_i(\myact{i},
\myact{j})$.
Yet another option is to follow \emph{Laplace's rule} which is to choose $\act^*$ only if
$\act^* \in \argmax_{a \in A}  \sum_{a'} \pi(a, a')$. Laplace's rule is to rationally choosing under a
\emph{flat belief} that assigns equal probability to all of the opponent's choices.

\todo[inline]{should we define NE, ESS/NSS here? dominance? we should include \emph{best
    reply}, because it is used in the definition of regret.}
...
\begin{definition}[Regret, regret minimization] \label{defn:regret} Given a simultaneous-move
...
\begin{example}[Coordination Game]
  ...
  equilibrium.\todo{explain Pareto-dominant NE; otherwise skip it?} $ \medsquare $
...
\todo[inline]{add results about regret: reg min is security choice on negative regret transform
(related to Figure 1b); regret choice equal to veridical in case of probabilistic beliefs and
maximization of expected gains}
