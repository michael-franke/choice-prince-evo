...
\date{current version: September 15th 2015}
...
\begin{definition}[Rational choice from maximization of expected utility] Let $S$ be a (finite)
  ...
  $\EU(a^*) = \sum_{s \in S} P(s) \ u(s, a^*)$: $a^* \in \text{argmax}_{a_i \in A} \EU(a^*)
  $. \todo{Give finite formulation? Everything we do here is finite}
\end{definition}
...
gains and losses). Similarly, tinkering with the notion of subjective utility, in particular by
including social preferences such as fairness, reciprocity or similar, allows us to describe as
rational empirically observed behavior in social dilemmas, such as the prisoners' dilemma or
...
rationality. \todo{speak about Dutch books here?} To begin with, if the DM's uncertainty is
about objective chance, it can be argued that subjective beliefs should conform (at least
roughly) to objective chance. Similarly, in certain situations it may make sense to postulate
on normative grounds that subjective preferences should be oriented towards tracking objective
utility. For instance, in their classic work \myalert{(Alchian, Friedman)} argue that profit
...
like to put this latter idea to the test. We 


\bigskip

 Since the work by vNM [cite], this view has been justified from
the ``intuitive rationality'' of the axioms on the subjective preference relation leading to
the representability of the decision maker as an expected utility maximizer. \todo{include
  axioms, or at least those that are mentioned as relevant later} While in the axiomatization
by [vNM] the probability of the outcomes is objectively given a priori, Savage54 provides a
larger set of axioms that represent the decision maker (DM) with both a subjective utility over
outcomes \textit{and} a \textit{subjective} probability over states of the world. Savage's
axioms constitute the foundation of subjective expected utility (SEU) theory, that is normally
assumed to define the rational behavior of economic agents.
