...
%   references Harley 1981!!!, O Conor \dots} Moreover,
...
\subsection{Choice mechanisms take2}

In the previous section we introduced choice rules generating behavior in a game as functions from a game to an action choice. In the general form, given a class of games $\mathcal{G}$, a choice rule is then thought as a function $c: \mathcal{G} \rightarrow A_i $. In this section instead we want to show that a more intelligible and perspicuous way of understanding behavior generating mechanisms may be as functions of a subjective preference and an epistemic state of the agents. Then, given a set of subjective preferences $ \Theta $ [@Paolo: minimally define subjective preferences somewhere above] and a set of epistemic states (or belief types) $E$, a choice mechanism is a function $\kappa: \Theta \times E \rightarrow A_i $. We will see in a moment that from this perspective we gain a more unified and structured insight on the choice rules presented above.

More
concretely, we focus on two \emph{preference types} and two \emph{belief types}. We view a preference type as a function $\tau: \mathcal{G} \rightarrow \Theta$, and a belief type as a general disposition to form beliefs about the co-player’s behavior. The two possible dispositions that we take account of are probabilistic (precise) belief and non-probabilistic (imprecise) belief, as we will explain in the next section.

We are most interested here in studying whether evolutionary pressure could support
non-veridical subjective preferences under certain conditions. We therefore consider two
\emph{preference types}: veridical type $\tau^\pi$, who uses the actual payoff function $\pi$ of the
actual game $G$, and regret type $\tau^{reg}$, who looks at the regret transformation of the
actual game $G$.

The choice mechanism that we use in this paper is \textit{maxmin expected utility} [GS89], for reasons that will be clear and justified in the following two sections.
