...
Formally, given a class $ \mathcal{G} $ of games, $\gamma: \mathcal{G} \rightarrow \Theta \times E$ is a function that assigns a subjective preference $\theta \in \Theta$ and an epistemic state (or belief type) $e \in E$ to any game $G \in \mathcal{G}$.\footnote{Given a game $G$, the set $ \Theta $ is the set of all subjective preferences, or subjective utility functions, on $G$. I.e., $\Theta$ is the set of all functions $\theta: A \rightarrow \mathbb{R}$.} 
Then, given a set of subjective
  preferences $ \Theta $ and
  a set of or belief types $E$, an action selection function is a function
  $\kappa: \Theta \times E \rightarrow A_i $, that associates any subjective representation of the game $ (\theta,e) \in \Theta \times E$ with an action $a^{*} \in A_i $. 

More concretely, we focus on two (\textit{subjective}) \emph{preference types} and two \emph{belief
  ...
  two possible dispositions that we take account of are precise probabilistic belief and
  imprecise belief, as we will explain in the next section.

Since we are most interested here in studying whether evolutionary pressure could
  support non-veridical subjective preferences, the two preference types that we consider are: the veridical type $\tau^\pi$, who uses the actual payoff function
  $\pi$ of the actual game $G$, and the regret type $\tau^{reg}$, who looks at the regret
  transformation of the actual game $G$.

Finally, the action selection function $\kappa$ that we use in this paper is \textit{maxmin expected
...
In decision theory, the imprecise probability model is in line with most of the representation results of decision making under uncertainty [GilboaSchmeidler89,KMM06], and seems justified by empirical observations too.
...
[[\noindent Definition \ref{def:expected fitness} gives the expected evolutionary fitness of player $(\tau, e)$ against player $(\tau', e')$ in game $G$.
...
Given a game $G \in \mathcal{G}$, a player $(\tau, e)$, and a player $(\tau', e')$, we define $\pi^G((\tau, e),(\tau', e')):= \pi^G(\kappa(\tau(G), e),\kappa(\tau'(G), e'))$. 

\end{definition}
[OF COURSE we will postpone the definition of evolutionary fitness, it is only momentarily here!]]]

It is then worth stressing here that $\kappa(\tau^{\pi}, \Delta(A_{-i}))$ turns out to be the actual payoff
