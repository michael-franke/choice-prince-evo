...
Table~\ref{tab:ExpectedFitness_4Types} gives an approximation of expected fitness, obtained
from averaging over randomly sampled $100,000$ games in $\mathcal{G}$ with
$a,b,c,d \in \lbrace 0, \dots, 10 \rbrace$. The number in each cell corresponds to the average
evolutionary fitness of the row player when matched with the column player. Whenever a player
type would not select a unique action in a given game, we recorded unbiased averages over
playing either.
...
Simulation results reflect Fact~\ref{fact:equivalence2x2} in that all encounters in which
types $\tau^{reg}, \Delta(A_{-i})$, $\tau^{reg}, \overline{\mu}$ or $\tau^{\pi},
\overline{\mu}$ are substituted for one another yield identical results. More interestingly,
Table~\ref{tab:ExpectedFitness_4Types} shows that the maximin strategy $\tau^{\pi},
\Delta(A_{-i})$ is strictly dominated by either of the three other strategies: in each row
column (i.e., for each kind of opponent), the maximin strategy is strictly worse than any of
the three behaviorally equivalent competitors. This has a number of interesting consequences.

Firstly, if we restrict attention to a population of radically uncertain players where only
epistemic type exists $\Delta(A_{-i})$, the regret type $ \tau^{reg} $ is the only
evolutionarily stable state; we expect selection that is driven by expected fitness to
invariably weed out maximin behavior in favor of regret minimization. In other words, in the
class of random games looked at here, regret minimization is a better choice rule than standard
maxmin from an evolutionary point of view.

Secondly, if we look at the competition between all four types represented in
Table~\ref{tab:ExpectedFitness_4Types}, $\tau^{reg}, \Delta(A_{-i})$ is no longer the only
evolutionarily stable type in this meta-game. Rather, given behavioral equivalence
(Fact~\ref{fact:equivalence2x2}), types $(\tau^{reg}, \Delta(A_{-i}))$,
$(\tau^{reg}, \overline{\mu})$, and $(\tau^{\pi}, \overline{\mu})$ are all neutrally stable
\citep{Maynard-Smith1982:Evolution-and-t}, while $(\tau^{\pi}, \Delta(A_{-i}))$ is strictly
dominated and so disfavored by fitness-based selection. Based on considerations of evolutionary
stability in this meta-game, we are drawn to conclude that maximin behavior is weeded out in
favor of a population with a random distribution of the remaining three types.
...
Interestingly, however, this variability is eradicated if we add a small mutation rate to the
dynamics. We assume a fixed, small mutation rate $\epsilon$ for the probability that a player's
preference type \emph{or} her epistemic type changes to another random preference type or
epistemic type. The probability that a player type randomly mutates into a completely different
player type with altogether different preference type and epistemic type would then be
$\epsilon^2$. With these assumptions about ``local mutations'', numerical simulations of the
(discrete time) \emph{replicator mutator dynamics} \citep{Nowak2006:Evolutionary-Dy} show that
for very small mutation rates almost all initial populations converge to a single fixed point
in which the majority of players are regret types. For instance, with $\epsilon = 0.001$,
almost all initial populations are attracted to a final distribution with proportions:
...
case with regret-based subjective preferences in our little toy example here. Since
regret-based subjective preferences are much better in connection with radical uncertainty than
veridical preferences are, we see a majority of regret-based preference types paired with
probabilistic epistemic types. In sum, component-wise mutation could have lead to a majority of
regret-based players even if they are behaviorally indiscernible from decision makers with
veridical preferences.

\subsection{Analytical results}
\label{sec:analytical-results}

Results based on the single meta-game in Table~\ref{tab:Utils-MetaGame}, which was obtained by
averaging over numerical simulations, are not fully general and possibly spoiled by random
fluctuations in the sampling procedure. Fortunately, for the case of symmetric $2 \times 2$
games, the main result that maximin types $(\tau^{\pi}, \Delta(A_{-i}))$ are strictly dominated
can also be shown analytically for generous general conditions on the nature of $P(G)$.

\begin{proposition} \label{proposition1}

  Let $\Lambda = \lbrace (\tau^{\pi}, \Delta(A_{-i})), (\tau^{reg}, \Delta(A_{-i})) \rbrace$ be
  the set of possible players $(\tau, e)$ in the population, and let $\mathcal{G}$ be the class
...
Corollary~\ref{corollary1} tells us that the results obtained for the meta-game in
Table~\ref{tab:Utils-MetaGame} hold more generally for any class of games in which the
probability of encountering a game is the product of independently sampling four identically
distributed payoff values. As before, player types $(\tau^{reg}, \Delta(A_{-i}))$,
$(\tau^{\pi}, \overline{\mu})$, and $(\tau^{reg}, \overline{\mu})$ will be neutrally stable,
and fitness-based selection will tend to weed out maximin-players and leave us with a
population of arbitrary frequency of $(\tau^{reg}, \Delta(A_{-i}))$,
$(\tau^{\pi}, \overline{\mu})$, and $(\tau^{reg}, \overline{\mu})$ players.


\section{Extensions}
\label{sec:extensions}

The basic results from the previous section demonstrate the main conceptual point that we
wanted to make: non-veridical subjective preference relations can persist under evolutionary
selection along a gradient of objective fitness. In particular, regret-based preferences can
...
models? Section~\ref{sec:more-types} first considers a further conceptually interesting
preference types that have been suggested in the recent literature. Section~XZY then looks at
the basic cast of player types, but considers the case of $n \times n$ games. Finally, to put
our results into the proper perspective, Section~XZY ends with a brief comparison of game
situations with cases of solitary decision making.
...
The approximations in Table~\ref{tab:ExpectedFitness_2x2_Full} confirm basic
intuitions. Averaging over randomly sampled games, everybody would like to have an altruistic
opponent and nobody would like to play against a competitive opponent. Moreover, altruistic
types come up strictly dominated by competitive types, but competitive types themselves are
always worse than any of the behaviorally equivalent types $(\tau^{reg}, \Delta(A_{-i}))$,
$(\tau^{\pi}, \overline{\mu})$, and $(\tau^{reg}, \overline{\mu})$, except against maximin
players $\tuple{\tau^{\pi}, \Delta(A)}$. This is a noteworthy results in the light of the fact
that evolving altruistic preferences have been shown to support cooperative behavior in a
single stage game \myalert{[CITE]}.\todo{@Paolo: check whether this is true, please!} In
contrast, averaging over payoffs in multiple stage games, like we do here, makes altruistic
preferences prime victims of evolutionary eradication.
...
all simulations of the (discrete-time) replicator dynamic end in populations consisting of only
these types in arbitrary proportion.

The following proposition confirms this result.\todo{rephrase to acknowledge all epistemic types}
...
Fact~\ref{fact:equivalence2x2} that regret minimization $\tuple{\tau^{reg}, \Delta(A)}$ is
behaviorally equivalent with expected regret minimization $\tuple{\tau^{reg}, \bar{\mu}}$ and
expected utility maximization $\tuple{\tau^{\pi}, \bar{\mu}}$. This is no longer true when we
look at arbitrary $n \times n$ games with $n \ge 2$. 

Table~\ref{tab:ExpectedFitness_10x10} therefore gives approximations for expected fitness in a
meta-game over a class over a class of $n \times n$ symmetric games where $n$ is randomly drawn
from $\set{2, \dots, 10}$. Concretely, the numbers in Table~\ref{tab:ExpectedFitness_10x10} are
