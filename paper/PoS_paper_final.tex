...
\title{Smart Representations: {R}ationality and Evolution in a Richer  Environment}
\author{Paolo Galeazzi $^{1}$ and Michael Franke $^{2}$\\
{\footnotesize
$^{1}$ ILLC, University of Amsterdam, The Netherlands; email: pagale87@gmail.com}\\
{\footnotesize
$^{2}$ Department of Linguistics, University of T\"{u}bingen, Germany; email: mchfranke@gmail.com}
}
\date{}
...
\footnotetext{The research of Paolo Galeazzi leading to these results
  ...
  Grant agreement No. 283963. Michael Franke’s work was supported by
  the Institutional Strategy of the University of T\"{u}bingen (DFG,
  ZUK 63). The authors are also grateful to Gerhard J{\"a}ger for
  funding this work. We want to thank all the participants in
  Decisions, Games and Logic ’15 workshop at London School of
  Economics, in LogiCIC workshop 2014 at the University of Amsterdam,
  and in Bridges 2 workshop at Rutgers University for the valuable
  feedback. Special thanks go to Pierpaolo Battigalli, Massimo
  Marinacci, Valerio Nispi Landi, Tommaso Orlando, Eric Pacuit, Erik
  Quaeghebeur, Rasmus Rendsvig, Teddy Seidenfeld, Peter Wakker, and
  Kevin Zollman for their attention and comments. We are also grateful
for the helpful comments we received by two anonymous reviewers.}
...
%Taken together, we presented a variety of plausible circumstances in which evolutionary competition between choice mechanisms on a larger class of games can favor non-veridical preference representations focusing on regret.
...
%
\node (zero) [] at (-.95,-.5) {$P(II)=0$};

%
%\node (s) [] at (1,0)[label=below:{$s$}] {|};
%
\node (s) [] at (1.5,0)
  [label=below:{$\frac{3s+t}{4}$}] {|};
%
%\node (t) [] at (3,0) [label=below:{$\overline{\lambda}$}] {|};
%
%\node (st) [] at (3,0)
 % [label=below:{$s + \nicefrac{\epsilon}{2}$}] {};
...
\node (3y) [] at (0,3)
  [label=right:{$I$}] {$\bullet$};
\node (4y) [] at (0,4)
  [label=left:{}] [label=below right:{}] {$\bullet$};
\node (5y) [] at (0,5)
  [label=left:{}] {$\bullet$};
%
\node (1yy) [] at (5,1)
...
\node (4yy) [] at (5,4)
   {$\bullet$};
\node (5yy) [] at (5,5)
  [label=left:{$II$}] [label=right:{$d$}] {$\bullet$};
%
...
%
\node (4y) [] at (-.5,2.6)
  [label=left:{}] [label=below right:{}] {$a^{\diamond}$};
\node (4y) [] at (5.6,3.6)
  [label=left:{}] [label=below right:{}] {$b^{\diamond}$};


%
\draw [-, very thick] (0,2.6) to (5,3.6);
%
\draw [-, very thick] (0,2) to (5,5);
%


\end{tikzpicture}
\caption{Game $G$.}
\label{subfig:coord-game-I-II-1-1}
\end{subfigure}
\hspace{3cm}
\begin{subfigure}[b]{0.3\textwidth}
...
%
\draw [-, very thick]  (8,0) to (13,0);
%
\draw [-, very thick] (8,0) to (8,5);

%
\draw [-, very thick] (13,0) to (13,5);
%


%
%\node (s2) [] at (9,0) [label=below:{$\underline{\lambda}$}] {|};
%
\node (s2) [] at (10.5,0)
  [label=below:{$\frac{s+3t}{4}$}] {|};
%
%\node (st2) [] at (11,0)
 % [label=below:{$s + \nicefrac{\epsilon}{2}$}] {};
%
%\node (t2) [] at (11,0) [label=below:{$\overline{\lambda}$}] {|};
%

\draw [dashed] (9,0) -- (9,5);
\draw [dashed] (11,0) -- (11,5);
...
\node (3y2) [] at (8,3) {$\bullet$};

\node (4y2) [] at (8,4)
  [label=right:{}] [label=left:{$\psi(a^{\diamond})$}] {$\bullet$};
\node (5y2) [] at (8,5) 
[label=right:{$II$}] [label=left:{$\psi(d)$}] {$\bullet$};
%
\node (1yy2) [] at (13,1)
  [label=right:{}] {$\bullet$};
\node (2yy2) [] at (13,2)
  [label=right:{$\psi(c)$}] {$\bullet$};
\node (2yy2) [] at (13,3)
 [label=right:{$\psi(b^{\diamond})$}] {$\bullet$};

\node (4yy2) [] at (13,4)
  [label=below left:{$I$}] {$\bullet$};
\node (5yy2) [] at (13,5)
  [label=left:{}] {$\bullet$};
%
...
%
\draw [-, very thick] (8,4) to (13,3);
%
\draw [-, very thick] (8,5) to (13,2);
%

\end{tikzpicture}
\caption{Game $\psi(G)$.}
\label{subfig:ant-game-I-II-1-1}
\end{subfigure}
\protect\caption{Examples of coordination game $C$ and corresponding anti-coordination
game $\psi(C)$.}
\label{fig:games-eq-I-II-1-1}
